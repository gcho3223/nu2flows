...
class IterativeNormLayer(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("n", T.tensor(n, dtype=T.int32))
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(min_value, T.Tensor):
            min_value = T.tensor(min_value, dtype=T.float32)
        if not isinstance(max_value, T.Tensor):
            max_value = T.tensor(max_value, dtype=T.float32)

        # Store the min and max values as buffers
        self.register_buffer("min_value", min_value.to(T.float32))
        self.register_buffer("max_value", max_value.to(T.float32))
